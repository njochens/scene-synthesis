...
class TriangleMesh:
    ...
    def to_svg_full(self, fill_color = "#FFFFFF", stroke_color = "black", width = 256, height = 256):
        svg = []
        svg.append('<?xml version="1.0" encoding="UTF-8"?>')
        svg.append(f'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" baseProfile="full" width="{width}" height="{height}" viewBox="0 0 {width} {height}">')
    ...
    def resize_and_pad(self, length, scale = 30):
        self.resize(scale)
        bbox = self.get_bbox()
        width = bbox[1] - bbox[0]
...
class Polygon:
    def __init__(self, corners = []):
        ...
        if len(corners) != 0:
            ...
            self.center = [0,0]
        self.rotation = 0
        self.corners = np.array(simplify_by_angle(spl_polygon(self.corners)))
...
class Object(Polygon):
    def __init__(self, name = "", category = "", category_detailed = "", id = -1, corners = [], color = "#000000", alpha = 0.0):
    ...
    def to_svg_line(self, fill_color = "#FFFFFF", stroke_color = "black"):
        ...
        for i in range(len(self.corners)):
            if i < len(self.corners) - 1:
                ...
                string = string + str(self.corners[i][0]) + "," + str(self.corners[i][1])
        return string + f' Z" fill="{self.color}" fill-opacity="{self.alpha}" stroke="none" stroke-width="0" />'            
    ...
    def to_json(self):
        ...
        json["category_detailed"] = self.category_detailed
        json["id"] = self.id
        return json
...
class Room(Polygon):
    ...
    def to_svg_file(self, fill_color = "#FFFFFF", stroke_color = "black", width = 256, height = 256):
        svg = []
        svg.append('<?xml version="1.0" encoding="UTF-8"?>')
        svg.append(f'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" baseProfile="full" width="{width}" height="{height}" viewBox="0 0 {width} {height}">')
        svg.append(f'<rect x="0" y="0" width="{width}" height="{height}" fill="white" />')
        svg.append(self.to_svg_line("#777777", "#777777"))
...
class TDFFurniture():
    ...
    def to_object(self):
        ...
        obj.category_detailed = self.category_detailed
